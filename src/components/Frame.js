...
const SidebarContent = ({ onClose, ...rest }) => {
  ...
  return (
    ...
    >
      ...
      {LinkItems.map((link) => {
        const isCurrent = link?.name?.toLowerCase() === current.toLowerCase();
        return (
          <NavItem
            key={link.name}
            icon={link.icon}
            isCurrent={isCurrent}
            color={isCurrent ? "#081466" : "blackAlpha.600"}
            fontFamily={"Jost"}
            fontWeight={"semibold"}
            fontSize={"15"}
            my={"1"}
...
const NavItem = ({ icon, children, isCurrent, ...rest }) => {
  return (
    <Link
      href="#"
      ...
      >
        {icon && (
          <Icon
            ...
            //   bg={""}
            // _groupHover={{
            //   color: isCurrent ? "#081466" : "blackAlpha.400",
            // }}
            as={icon}
          />
        )}
